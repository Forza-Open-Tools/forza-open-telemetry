...
const lapClass = computed(() => store.lapColorClasses[store.selectedLapIndex]);
...
<template>
  <div>
    ...
    <div v-for="lap in store.selectedRace!.laps" :key="lap.lap" class="flex cursor-pointer items-center"
      :class="{ 'font-bold': (lap.lap - 1) === store.selectedLapIndex }" @click="onLapClick(lap.lap)">
      <div class="w-4 h-4 border border-gray-800" :class="lapClass.bg" />
